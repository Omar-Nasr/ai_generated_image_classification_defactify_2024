...
def train_classifier(train_data_dir,checkpoint_path,num_epochs=10,val_data_dir=None,val_labels=None,val=False,batch_sz=16,task="Binary",model_name="swin",optimizer_name="adam",use_fourier=False,lr=1e-7,lr2=1e-4,fine_tune=False,trial=None,dropout_rate=0.18,num_classes=6,freeze_number=0,classical_ml=False,k_fold=False,passed_model=None,passed_classifier=None,test_run=False,tpu_training=False):
    ...
    if(val==True):
        ...
        if(classical_ml==True):
            ...
            model_trained = train_model(model,criterion,optimizer,optimizer2,scheduler,scheduler2,train_dataloader,classifier,num_epochs,checkpoint_path,task,use_fourier=use_fourier,model_name=model_name,val_dataloader=val_dataloader,batch_sz=batch_sz,fine_tune=fine_tune,test=test_run,tpu_training=tpu_training)
        else:
            ...
            for fold, (train_idx, test_idx) in enumerate(kf.split(img_dataset)):
                ...
                model,classifier =  train_model(model,criterion,optimizer,optimizer2,scheduler,scheduler2,train_dataloader,classifier,num_epochs,checkpoint_path,task,use_fourier=use_fourier,model_name=model_name,val_dataloader=test_dataloader,batch_sz=batch_sz,fine_tune=fine_tune,test=test_run,tpu_training=tpu_training)
        ...
        model_trained = train_model(model,criterion,optimizer,optimizer2,scheduler,scheduler2,train_dataloader,classifier,num_epochs,checkpoint_path,task,use_fourier=use_fourier,model_name=model_name,batch_sz=batch_sz,fine_tune=fine_tune,tpu_training=tpu_training)
